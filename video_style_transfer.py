...
def video_style_transfer(config):
    ...
    frame_height, frame_width, _ = cv2.imread(os.path.join(output_dir, "transferred_frames", "transferred_frame-00000001.jpg")).shape
    # synthesize video using transferred content frames
    cv2_fourcc = cv2.VideoWriter_fourcc(*'mp4v')
    video_writer = cv2.VideoWriter(output_video_path, cv2_fourcc, fps, (frame_width, frame_height), True)
